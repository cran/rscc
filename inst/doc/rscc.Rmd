...
If so, the expression is kept for further analysis. The name of the list items in `prgs` is then `filename[number]`. For example, you could access the expression named `prgs[["aa.R[1]"]]`.
...
into a data frame, where the first column is the row index, the second column the column index and the third 
column the similarity coefficient. If `decreasing` is `TRUE` (default), the data frame is sorted in descending order by the third column.
...
## Step 4: Detailed analysis of the source code
